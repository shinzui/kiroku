...
evidence:
  - kind: test
    ...
    proves: Applies the manifest against an ephemeral PostgreSQL, asserts the eight-file ordering, that every legacy SQL payload matches migrations.lock byte-for-byte, and that a rerun is AlreadyApplied.
  - kind: module
