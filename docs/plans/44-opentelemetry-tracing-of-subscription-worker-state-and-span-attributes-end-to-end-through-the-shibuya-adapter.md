...
- [ ] M2 — `shibuya-kiroku-adapter`: populate `Envelope.attributes` with kiroku identity (subscription name, consumer-group member, event type, global position), threaded from the adapter config; adapter test asserts the attributes are present.
...
- Add a small attribute-source value (for example `data KirokuEnvelopeAttrs = KirokuEnvelopeAttrs { subName :: Text, member :: Maybe Int }`) and thread it from `kirokuAdapter` (which knows the subscription name from `KirokuAdapterConfig`) and from `kirokuConsumerGroupProcessors` (which knows the name and assigns each per-member processor its member index — see EP-42's `ProcessorId "<name>-member-<m>"` construction in `Shibuya/Adapter/Kiroku.hs`) into the conversion.
