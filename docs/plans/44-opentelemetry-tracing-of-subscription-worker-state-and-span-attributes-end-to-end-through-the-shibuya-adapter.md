...
- **Two post-ship performance refinements (code review, 2026-05-31).** A review of the committed EP-5 code surfaced two non-blocking but worthwhile costs, both since fixed; behavior and test counts are unchanged (`kiroku-otel-test` 13/0, `shibuya-kiroku-adapter-test` 20/0):
  - *The span tracer held its `MVar` across span syscalls.* `withKey`/`dropKey` in `Kiroku.Otel.Subscription` originally ran the whole span-IO closure (`createSpan`/`endSpan`, each a `clock_gettime` + allocation) inside `modifyMVar_`, so a single global lock serialized every worker — and a consumer group runs one worker per member on separate threads. The critical section is now narrowed to two short pure `O(log n)` map operations (read the key's state, commit the result) with the span IO performed *outside* the lock. This is safe because each `SpanKey` is single-writer: every `(name, member)` is emitted by exactly one worker thread and the callback is synchronous, so a key's `OpenState` cannot change between read and commit; only the shared `Map` needs guarding. The biggest-volume traced event (`Fetched`) is per-batch, so this was latent rather than acute, but the global-lock-across-syscalls shape is now gone.
  - *The adapter rebuilt the whole attribute map per event.* `toEnvelope` called `kirokuEnvelopeAttributes` for every `RecordedEvent`, re-allocating the constant subscription-name (and member) `Attribute` values on the per-event path even when no OTel exporter was wired up. `KirokuEnvelopeAttrs` is now a `newtype` holding the constant attributes precomputed once (built in `kirokuAdapter` via the new `kirokuEnvelopeAttrs` smart constructor); the per-event path only inserts this event's `kiroku.event.type` and `kiroku.event.global_position` into that base map. (Cardinality note: these high-cardinality numeric attributes are fine on *spans* — the concern would only apply to metrics, which stay deferred.)
