...
- Decision: Fix F6 (unbounded broadcast) by replacing `TChan` + `dupTChan` with a per-subscriber bounded `TBQueue` registry inside the publisher. Default `queueCapacity = 16` batches (~16,000 events at the default publisher batch size), default `overflowPolicy = DropSubscription` (cancel the slow subscriber and surface a structured error via `Async.waitCatch`).
...
- Decision: Implement F6 inside `kiroku-store` rather than delegating to `shibuya-core`'s ingester/inbox machinery (which already provides bounded, block-publisher backpressure for any `Adapter`).
  Rationale: Shibuya provides backpressure between an *adapter source* and a *handler* — its bounded inbox sits *downstream* of `subscriptionStream`, which itself sits downstream of the kiroku-store worker, which sits downstream of the publisher's broadcast `TChan` + per-subscriber `dupTChan`. The unbounded fan-out lived at the publisher layer; nothing shibuya does can reach it. Even pre-F6, the `subscriptionStream` + shibuya path was buggy under load — the shibuya inbox would saturate, the `subscriptionStream` TBQueue would fill, the worker would stop draining its `dupTChan`, and the `dupTChan` would grow without bound. Adding shibuya in front did not protect against the OOM. F6 had to land at the fan-out layer where the unbounded queue actually lived. A second consideration: shibuya's ingester uses block-publisher backpressure (one bounded inbox per adapter-handler pair, no head-of-line blocking because adapters are independent). Kiroku's topology is one-publisher-fanning-out-to-many-subscribers, so block-publisher would let one slow subscriber stall delivery for every other subscriber. The chosen `DropSubscription` default is policy-distinct from shibuya for this structural reason. Kiroku-via-shibuya consumers now get *three* layers of bounded queueing: F6's per-subscriber publisher queue, the `subscriptionStream` TBQueue, and shibuya's inbox.
  Date: 2026-04-29
...
- Decision: Keep both the push-style `subscribe` API and the pull-style `subscriptionStream` API as public surface; defer the question of collapsing them into a streaming-only surface (with supervision/policies/metrics delegated to `shibuya-core`).
  Rationale: There is a legitimate architectural argument that kiroku-store should expose only `subscriptionStream` and let consumers compose with shibuya for everything else (`Adapter` API, supervision, ack semantics, metrics, multi-processor harnessing). That would let kiroku-store drop `OverflowPolicy`, `SubscriptionOverflowed`, the publisher's per-subscriber registry's policy machinery, and the `withSubscription` bracket variants, on the grounds that shibuya already supplies equivalents at the framework layer. EP-3 chose not to pursue this in M2 because: (a) `subscriptionStream` itself uses `subscribe` internally — collapsing the surface is a refactor, not just a deletion; (b) F6 had to fix the unbounded fan-out at the publisher layer regardless of which API was public, so the bounded-queue work is not wasted; (c) consumers that do not use shibuya (tests, benchmarks, simple projection scripts) would lose the simpler push API; (d) the topology mismatch (one-publisher-many-subscribers vs one-adapter-one-handler) means shibuya's per-adapter inbox model does not map cleanly onto kiroku's fan-out without an intermediate adapter-per-subscription layer that someone still has to write. Revisit when (1) the only known consumer is shibuya, (2) the multi-tenancy work in EP-4 has settled, or (3) someone files a concrete request to drop one API.
  Date: 2026-04-29
