...
* Requires `kiroku-store ^>=0.6`, whose exported `Store` effect now offers the
  visible global head position read. No `shibuya-kiroku-adapter` API or runtime
  behavior changed.
...
* Requires `kiroku-store ^>=0.4`, whose exported `Store` effect now supports
  durable subscription checkpoint inventory reads. No
  `shibuya-kiroku-adapter` API or runtime behavior changed.
...
#### Shibuya 0.6 and OpenTelemetry 1.0 semantics
...
#### Ack decisions now drive Kiroku checkpointing (plan 40)
...
#### Kiroku identity on envelope attributes for end-to-end tracing (MasterPlan 6 EP-5)
...
#### `defaultKirokuAdapterConfig` smart constructor
...
#### Consumer groups as a single partitioned subscription (plan 42)
...
#### Event-type filter and selector forwarding (plan 43)
